...
y_hat =theta1*std_data +theta0 

slope, intercept, r_value, p_value, std_err = stats.linregress(std_data,y2)


plt.xlim([min(std_data),max(std_data)])
plt.ylim([min(y2),max(y2)])
print(y2, '\n', y_hat)
plt.plot(std_data, y2, '*')
plt.plot(std_data, y_hat)
plt.plot([min(std_data),max(std_data)],[intercept+slope*min(std_data),intercept+slope*max(std_data)])
